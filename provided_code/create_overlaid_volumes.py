...
def process_patient_overlay(patient_folder: Path, output_root: Path):
    ...
    # NEW INPUT PATH: 10-channel volume from transformed data
    struct_10ch_input_path = patient_folder / f'{patient_id}_struct_10ch.npy'
    ...
    # NEW OUTPUT PATH: 10-channel volume in overlaid-data folder
    ...
    if overlay_output_path.exists() and dose_output_path.exists() and struct_10ch_output_path.exists():
        return
    ...
    # --- D. Save Overlay and Copy Dose/10-channel Volume ---
    try:
        # 1. Save the newly created overlay volume
        np.save(overlay_output_path, overlay_volume)

        # 2. Copy the Dose file (implicitly by loading and saving)
        # Check if the input file exists before loading.
        if dose_input_path.exists():
            dose_volume = np.load(dose_input_path)
            np.save(dose_output_path, dose_volume)
        else:
             print(f"\nWARNING: Dose file not found for {patient_id} at {dose_input_path}. Skipping copy.")

        # 3. Copy the 10-channel Structure file (implicitly by loading and saving) - NEW COPY LOGIC
        if struct_10ch_input_path.exists():
            struct_10ch_volume = np.load(struct_10ch_input_path)
            np.save(struct_10ch_output_path, struct_10ch_volume)
        else:
             print(f"\nWARNING: 10-channel structure file not found for {patient_id} at {struct_10ch_input_path}. Skipping copy.")

...
# The list of ROIs used for the full 10-channel encoding
# The order here determines the channel index (0 to 9)
FULL_ROI_LIST = [
    "PTV70", "PTV63", "PTV56",
...
# Unique 3-Channel Encoding Scheme (Disjoint Axes) - Used for the 3-channel volume
COLOR_ENCODING = {
...
def load_file_from_csv(file_path: Path):
    ...
    try:
        ...
        # Check if the file name matches any of the ROI names in the encoding
        is_mask_file = any(name in file_path.name for name in FULL_ROI_LIST)
        ...
        # print(f"Error loading {file_path.name}: {e}") # Debugging
        return None
...
def create_10channel_volume(patient_folder: Path) -> np.ndarray:
    """
    Reads individual structure masks and combines them into a 10-channel volume,
    where each channel represents one ROI (binary mask).
    The channel order is determined by FULL_ROI_LIST.
    Output shape: (D, H, W, 10) uint8.
    """
    # 10 is the length of FULL_ROI_LIST
    channel_count = len(FULL_ROI_LIST)
    # Initialize the 4D volume (128, 128, 128, 10)
    struct_10ch_volume = np.zeros((*CT_SHAPE_3D, channel_count), dtype=np.uint8)

    for i, roi_name in enumerate(FULL_ROI_LIST):
        # Load the binary mask for the current ROI
        mask_3d = try_load_mask(patient_folder, roi_name, CT_SHAPE_3D)

        if mask_3d is not None:
            # Place the 3D mask into the corresponding channel (i)
            struct_10ch_volume[..., i] = mask_3d

    return struct_10ch_volume
...
def process_patient(patient_folder: Path, output_root: Path):
    ...
    # NEW: 10-channel structure output path
    struct_10ch_output_path = output_patient_folder / f'{patient_id}_struct_10ch.npy'

    # CHECK: Skip if ALL output files exist. (Updated to include the new 10ch file)
    if ct_output_path.exists() and struct_output_path.exists() and \
       dose_output_path.exists() and struct_10ch_output_path.exists():
        return

    # --- B. Load CT, Dose, and Create Structure Volumes ---
    try:
        # 1. Load CT Volume (128x128x128)
        ct_volume = load_scalar_volume(patient_folder, "ct", CT_SHAPE_3D)

        # 2. Load Dose Volume (128x128x128)
        ...
        # 4. Generate the 10-channel structure volume (128x128x128x10) - NEW CREATION
        struct_10ch_volume = create_10channel_volume(patient_folder)
    ...
    try:
        ...
        # NEW: Save the 10-channel structure volume
        np.save(struct_10ch_output_path, struct_10ch_volume)
